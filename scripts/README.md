...
This directory contains the complete analysis pipeline for the TransCisPredict framework - a comprehensive approach to protein expression weights estimation that incorporates both _cis_- and _trans_- variants for proteome-wide association studies (PWAS). The pipeline consists of 8 sequential steps, each implemented as a standalone R script with standardized configuration sections.
...
Processes raw OLINK NPX(normalized protein expression) data, performs quality control, and creates individual protein expression files.
...
Removes covariate effects (age, sex, age×sex, BMI, genetic PCs 1-20) from protein expression to generate NPX residuals for genetic analysis.
...
Identifies genomic regions with significant genetic signal using FDR-based selection to focus in the downstream analysis.
...
Performs cross-validation to evaluate multiple statistical methods (BayesR, SuSiE, LASSO, Elastic Net) for protein prediction using both _cis_- and _trans_- genetic variants.

### Step 5: CV Evaluation
**Scripts**: 
- [step5a_evaluate_cv_performance.R](https://github.com/statgenetics/TransCisPredict/blob/main/scripts/step5a_evaluate_cv_performance.R) - Evaluates cross-validation performance for four methods per protein
...
- [step5c_summarize_all_methods.R](https://github.com/statgenetics/TransCisPredict/blob/main/scripts/step5c_summarize_all_methods.R) - Generate a comprehensive method summary for all proteins (optional)
...
Estimates the weights using the complete reference dataset and "optimal" method identified for each protein.

### Step 7: Population Prediction
**Scripts**:
- [step7a_predict_npx_population.R](https://github.com/statgenetics/TransCisPredict/blob/main/scripts/step7a_predict_npx_population.R) - Predicts protein levels for the target sample
- [step7b_combine_npx_files.R](https://github.com/statgenetics/TransCisPredict/blob/main/scripts/step7b_combine_npx_files.R) - Combines individual protein predictions into a single matrix

Applies trained weights to predict NPX residual levels for the target sample (without measured proteomic data).
...
Performs proteome-wide association studies using generalized estimating equations (GEE) to test associations between predicted NPX levels and complex traits.
...
- [pqtl_weights.R](https://github.com/statgenetics/TransCisPredict/blob/main/scripts/utilities/pqtl_weights.R) - Statistical method implementations (BayesR, SuSiE, LASSO and Elastic Net)
